...
const ProductDetail = () => {
  ...
  useEffect(() => {
    ...
    const fetchProductData = async () => {
      try {
        const { data } = await axios.get(`https://tunimarket.onrender.com/api/products/${productId}`);
        setProduct(data);
      } catch (error) {
  ...
  const handleToggleFavorite = async () => {
    const userEmail = window.localStorage.getItem('email');
    try {
      await axios.post('https://tunimarket.onrender.com/api/favorites', {
        email: userEmail,
  ...
  return (
    <div className="product-detail-wrapper">
      ...
      <Helmet>
        ...
        <meta name="keywords" content={`${product.name}, ${product.category}, buy ${product.name}, ${product.location}`} />
        <link rel="canonical" href={`https://tunimarket.onrender.com/product/${product.id}`} />
      </Helmet>
