...
function ProductFeed({ filteredProducts, handleProductClick }) {
  ...
  // Preload the first product image to improve LCP
  const preloadImageUrl = visibleProducts[0]?.imageUrl;
  ...
  return (
    <div>
      <Helmet>
        ...
        {/* Preload the first product image for LCP */}
        {preloadImageUrl && (
          <link
            rel="preload"
            href={preloadImageUrl}
            as="image"
            type="image/jpeg" // Change to appropriate image type if necessary
