...
function App() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const { data: productsData } = await axios.get('https://tunimarket.onrender.com/api/products');
        const { data: categoriesData } = await axios.get('https://tunimarket.onrender.com/api/categories');
  ...
  useEffect(() => {
    ...
    if (match && match[1]) {
      ...
      if (product) {
        ...
        axios
          .get(`https://tunimarket.onrender.com/api/products/${productId}`)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post('https://tunimarket.onrender.com/api/products', data, { headers: { 'Content-Type': 'multipart/form-data' } });
      setFormData({ name: '', price: 0, image: null, location: '', stock: 1, category: '', username: '', email: '' });
      setShowForm(false);
      const { data: updatedProducts } = await axios.get('https://tunimarket.onrender.com/api/products');
  ...
  const handleSubmitOrder = async (paymentOption, additionalDetails) => {
    ...
    try {
      await axios.post('https://tunimarket.onrender.com/api/orders', orderDetails);
      setShowCart(false);
  ...
  const onLoginSuccess = async () => {
    ...
    if (email) {
      try {
        ...
        const { data: allProducts } = await axios.get('https://tunimarket.onrender.com/api/products');
  ...
  const handleFavorite = async () => {
    ...
    try {
      const { data: favoriteProducts } = await axios.get(`https://tunimarket.onrender.com/api/getfavorites?email=${userEmail}`);
