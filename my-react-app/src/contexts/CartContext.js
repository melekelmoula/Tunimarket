...
export const CartProvider = ({ children }) => {
  ...
  const fetchCart = async (email) => {
    try {
      const response = await axios.get(`https://tunimarket.vercel.app//api/cart`, { params: { email } });
...
if (existingProductIndex >= 0) {
  setCartItems((prevItems) => {
    ...
    return updatedItems;
      ...
      if (email != null) {
        try {
          await axios.post('https://tunimarket.vercel.app//api/cart', {
            email,
  ...
  const removeFromCart = async (productId) => {
    ...
    if (email != null) {
      try {
        await axios.post('https://tunimarket.vercel.app//api/cart/remove', {
          email,
          productId,
        });
      } catch (error) {
